...
class TransformerBlock:
  ...
  def attn(self, x):
    ...
    return attention.reshape(shape=(x.shape[0], -1, self.num_heads * self.head_size)).linear(
      *self.out
    )
  ...
  def __call__(self, x):
    if self.prenorm:
      ...
      x = x + self.act(x.layernorm().linear(*self.ln2).linear(*self.ff1)).linear(*self.ff2).dropout(
        self.dropout
      )
    else:
