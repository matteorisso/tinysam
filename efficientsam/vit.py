  ...
  def load_from_pretrained(m):
    ...
    for i in range(12):
      m.tbs[i].query[0].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/query/kernel"].reshape(
          m.embed_dim, m.embed_dim
        )
      )
      m.tbs[i].query[1].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/query/bias"].reshape(
          m.embed_dim
        )
      )
      m.tbs[i].key[0].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/key/kernel"].reshape(
          m.embed_dim, m.embed_dim
        )
      )
      m.tbs[i].key[1].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/key/bias"].reshape(
          m.embed_dim
        )
      )
      m.tbs[i].value[0].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/value/kernel"].reshape(
          m.embed_dim, m.embed_dim
        )
      )
      m.tbs[i].value[1].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/value/bias"].reshape(
          m.embed_dim
        )
      )
      m.tbs[i].out[0].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/out/kernel"].reshape(
          m.embed_dim, m.embed_dim
        )
      )
      m.tbs[i].out[1].assign(
        dat[f"Transformer/encoderblock_{i}/MultiHeadDotProductAttention_1/out/bias"].reshape(
          m.embed_dim
        )
      )
